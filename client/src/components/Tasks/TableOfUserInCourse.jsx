...
const TableOfUserInCourse =  ({setSelectStudent ,selectStudent}) => {
...
// console.log(students)
  ...
  const handleCheckboxItems = () => {
      ...
      students.forEach((item, idx) => {
          selectStudent[item.user_id] = !areAllChecked
...
//  console.log(selectStudent)
  ...
  const handleCheckboxChange = (e, idx) => {
      setAllChecked(false)
      setSelectStudent({ ...selectStudent, [e.target.name]: e.target.checked })
  }

  useEffect(() => {
      // Set properties with false value
      students.forEach((item, idx) => {
          selectStudent[item.user_id] = true
          setSelectStudent({ ...selectStudent })
      })
  }, [])
  ...
  useEffect(() => {
      ...
      const checkboxItemsVal = Object.values(selectStudent)
      const checkedItems = checkboxItemsVal.filter(item => item == false)
      if (checkedItems.length == students.length) setAllChecked(false)
  }, [selectStudent])
  ...
  return (
    <div className="max-w-screen-xl  px-4 md:px-8 ">
      ...
      <div className="mt-12 shadow-sm border rounded-lg h-[22rem] overflow-y-scroll">
              ...
              <th className="py-3 px-6 flex items-center gap-x-4 ">
                <div>
                  ...
                  <label
                    ...
                    className="relative flex w-5 h-5 bg-white peer-checked:bg-fluencia-dark-purple rounded-md border ring-offset-2 ring-indigo-600 duration-150 peer-active:ring cursor-pointer after:absolute after:inset-x-0 after:top-[3px] after:m-auto after:w-1.5 after:h-2.5 after:border-r-2 after:border-b-2 after:border-white after:rotate-45"
                  ></label>
                </div>
                Username
              </th>
                ...
                <td className="px-6 py-4 whitespace-nowrap flex items-center gap-x-4">
                  <div>
                    <input
                      ...
                      name={item.user_id}
                      className="checkbox-item peer hidden"
                      checked={areAllChecked || selectStudent[item.user_id]}
                      onChange={(e) => handleCheckboxChange(e, item.user_id)}
                    />
                    <label
                      htmlFor={item.user_id}
                      className="relative flex w-5 h-5 bg-white peer-checked:bg-fluencia-dark-purple rounded-md border ring-offset-2 ring-white duration-150 peer-active:ring cursor-pointer after:absolute after:inset-x-0 after:top-[3px] after:m-auto after:w-1.5 after:h-2.5 after:border-r-2 after:border-b-2 after:border-white after:rotate-45"
                    ></label>
                  </div>
...
export default TableOfUserInCourse;